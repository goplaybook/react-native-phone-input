...
export default class PhoneInput extends Component {
  ...
  constructor(props, context) {
    ...
    if (countriesList) Country.setCustomCountriesData(countriesList);
  ...
  onChangePhoneNumber = (number) => {
    const actionAfterSetState = this.props.onChangePhoneNumber && (() => {
      const valid = PhoneNumber.isValidNumber(number, this.state.iso2);
      this.props.onChangePhoneNumber(number, valid)
    });
  ...
  selectCountry = (iso2) => {
    if (this.state.iso2 !== iso2) {
      ...
      if (countryData) {
        this.setState(
          { iso2, formattedNumber: `+${countryData.dialCode}` },
          () => this.props.onSelectCountry && this.props.onSelectCountry(iso2)
        );
      }
    }
  }
